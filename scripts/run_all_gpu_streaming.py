#!/usr/bin/env python3
"""
Run 5 sequential queries to GPT-5.4-pro via OpenAI Responses API (streaming)
for deep regional GPU usage research.

Streaming keeps the connection alive and avoids timeout issues
that occur with non-streaming calls on gpt-5.4-pro + web_search_preview.
"""
...
def run_query_streaming(idx, query_info):
    """Run a single query using streaming to avoid timeout issues."""
    print(f"\n{'='*70}", flush=True)
    ...
    try:
        ...
        for event in stream:
            ...
            # Log event types periodically to show progress during web search phase
            if event_count <= 5 or event_count % 20 == 0:
                elapsed = time.time() - start
                print(f"  [{elapsed:.0f}s] event #{event_count}: {etype}", flush=True)

            if hasattr(event, 'delta') and event.delta:
                if first_text_time is None:
                    first_text_time = time.time() - start
                    print(f"  [{first_text_time:.0f}s] First text chunk received!", flush=True)
                text_parts.append(event.delta)
                chunk_count += 1
                if chunk_count % 50 == 0:
                    elapsed = time.time() - start
                    print(f"  [{elapsed:.0f}s] ...{chunk_count} text chunks", flush=True)

        result = ''.join(text_parts)
        elapsed = time.time() - start
        print(f"  -> OK: {len(result)} chars, {chunk_count} text chunks, {event_count} total events in {elapsed:.1f}s", flush=True)
        return result
        ...
        import traceback
        traceback.print_exc()
        # Return partial result if we got anything
        if text_parts:
            partial = ''.join(text_parts)
            print(f"  -> Returning partial result: {len(partial)} chars", flush=True)
            return partial + f"\n\n[ERROR: Query terminated after {elapsed:.1f}s: {e}]"
        return f"[Error: {type(e).__name__}: {e}]"


def main():
    print(f"{'='*70}", flush=True)
    print(f"Regional GPU Usage Research with {MODEL} (streaming)", flush=True)
    ...
    for idx, q in enumerate(queries):
        result = run_query_streaming(idx, q)
        results.append((q, result))

        # Save incremental results after each query
        save_results(results)
        print(f"  Incremental save done ({len(results)}/5 queries)", flush=True)

        # Small delay between queries
        if idx < len(queries) - 1:
            print("  Waiting 5s before next query...", flush=True)
            time.sleep(5)

    print(f"\n{'='*70}", flush=True)
    print(f"All 5 queries complete!", flush=True)
